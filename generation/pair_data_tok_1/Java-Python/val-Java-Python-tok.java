...
public static void main ( String [ ] args ) { int n = 5 , k = 2 ; System . out . printf ( " Value ▁ of ▁ C ( % d , ▁ % d ) ▁ is ▁ % d ▁ " , n , k , binomialCoeff ( n , k ) ) ; } }
import java . util . * ; class GFG {
static int nCr ( int n , int r ) { if ( r > n ) return 0 ;
if ( n - r > r ) r = n - r ;
for ( int i = 1 ; i <= n ; i ++ ) SPF [ i ] = i ;
for ( int i = 4 ; i <= n ; i += 2 ) SPF [ i ] = 2 ; for ( int i = 3 ; i * i < n + 1 ; i += 2 ) {
if ( SPF [ i ] == i ) {
for ( int j = i * i ; j < n + 1 ; j += i ) if ( SPF [ j ] == j ) { SPF [ j ] = i ; } } }
Map < Integer , Integer > prime_pow = new HashMap < > ( ) ;
for ( int i = r + 1 ; i < n + 1 ; i ++ ) { int t = i ;
while ( t > 1 ) { prime_pow . put ( SPF [ t ] , prime_pow . getOrDefault ( SPF [ t ] , 0 ) + 1 ) ; t /= SPF [ t ] ; } }
for ( int i = 1 ; i < n - r + 1 ; i ++ ) { int t = i ;
while ( t > 1 ) { prime_pow . put ( SPF [ t ] , prime_pow . get ( SPF [ t ] ) - 1 ) ; t /= SPF [ t ] ; } }
long ans = 1 , mod = 1000000007 ;
for ( int i : prime_pow . keySet ( ) )
ans = ( ans * pow ( i , prime_pow . get ( i ) , mod ) ) % mod ; return ( int ) ans ; }
