...
int averageOdd ( int n ) { if ( n % 2 == 0 ) { printf ( " Invalid ▁ Input " ) ; return -1 ; } return ( n + 1 ) / 2 ; }
...
void printTrinomial ( int n ) { int dp [ MAX ] [ MAX ] = { 0 } ;
for ( int i = 0 ; i < n ; i ++ ) {
for ( int j = - i ; j <= 0 ; j ++ ) cout << TrinomialValue ( dp , i , j ) << " ▁ " ;
for ( int j = 1 ; j <= i ; j ++ ) cout << TrinomialValue ( dp , i , j ) << " ▁ " ; cout << endl ; } }
int main ( ) { int n = 4 ; printTrinomial ( n ) ; return 0 ; }
int isPowerOf2 ( char * str ) { int len_str = strlen ( str ) ;
int num = 0 ;
if ( len_str == 1 && str [ len_str - 1 ] == '1' ) return 0 ;
while ( len_str != 1 str [ len_str - 1 ] != '1' ) {
if ( ( str [ len_str - 1 ] - '0' ) % 2 == 1 ) return 0 ;
for ( int i = 0 , j = 0 ; i < len_str ; i ++ ) { num = num * 10 + str [ i ] - '0' ;
if ( num < 2 ) {
if ( i != 0 ) str [ j ++ ] = '0' ;
continue ; } str [ j ++ ] = ( int ) ( num / 2 ) + '0' ; num = ( num ) - ( num / 2 ) * 2 ; } str [ j ] = ' \0' ;
len_str = j ; }
return 1 ; }
int main ( ) { char str1 [ ] = "12468462246684202468024" "6842024662202000002" ; char str2 [ ] = "1" ; char str3 [ ] = "128" ; printf ( " % d % d % d " , isPowerOf2 ( str1 ) , isPowerOf2 ( str2 ) , isPowerOf2 ( str3 ) ) ; return 0 ; }
...
#include <stdio.h> NEW_LINE int multiplyBySevenByEight ( unsigned int n ) {
return ( ( n << 3 ) - n ) >> 3 ; }
int main ( ) { unsigned int n = 15 ; printf ( " % u " , multiplyBySevenByEight ( n ) ) ; return 0 ; }
