...
import java . awt . * ; import java . util . * ; class GFG {
...
import java . lang . * ; import java . util . * ; class GFG {
...
import java . util . Stack ; class GFG {
...
if ( S . empty ( ) ) { break ; }
top = S . peek ( ) ; }
if ( S . empty ( ) ) { S . push ( A [ i ] ) ; } else { top = S . peek ( ) ;
if ( A [ i ] < top ) { S . push ( A [ i ] ) ;
} else {
return false ; } } } else {
S . push ( A [ i ] ) ; } }
return true ; }
public static void main ( String [ ] args ) { int A [ ] = { 4 , 1 , 2 , 3 } ; int N = A . length ; if ( check ( A , N ) ) { System . out . println ( " YES " ) ; } else { System . out . println ( " NO " ) ; } } }
import java . util . Stack ; public class GFG {
static int countMinReversals ( String expr ) { int len = expr . length ( ) ;
if ( len % 2 != 0 ) return - 1 ;
Stack < Character > s = new Stack < > ( ) ; for ( int i = 0 ; i < len ; i ++ ) { char c = expr . charAt ( i ) ; if ( c == ' } ' && ! s . empty ( ) ) { if ( s . peek ( ) == ' { ' ) s . pop ( ) ; else s . push ( c ) ; } else s . push ( c ) ; }
int red_len = s . size ( ) ;
int n = 0 ; while ( ! s . empty ( ) && s . peek ( ) == ' { ' ) { s . pop ( ) ; n ++ ; }
...
if ( len % 2 != 0 ) { return - 1 ; } int left_brace = 0 , right_brace = 0 ; for ( int i = 0 ; i < len ; i ++ ) { char ch = expr . charAt ( i ) ;
if ( ch == ' { ' ) { left_brace ++ ; }
else { if ( left_brace == 0 ) { right_brace ++ ; } else { left_brace -- ; } } } ans = ( int ) ( Math . ceil ( ( 0.0 + left_brace ) / 2 ) + Math . ceil ( ( 0.0 + right_brace ) / 2 ) ) ; return ans ; }
public static void main ( String [ ] args ) { String expr = " } } { { " ; System . out . println ( countMinReversals ( expr ) ) ; } }
class GFG {
...
import java . io . * ; class SmallPrime {
...
import java . util . * ; import java . lang . * ; public class GfG {
...
import java . io . * ; import java . util . * ; class KeyComparator implements Comparator < Key > {
int freq ; char ch ; Key ( int val , char c ) { freq = val ; ch = c ; } } class GFG { static int MAX_CHAR = 26 ;
static void rearrangeString ( String str ) { int n = str . length ( ) ;
int [ ] count = new int [ MAX_CHAR ] ; for ( int i = 0 ; i < n ; i ++ ) count [ str . charAt ( i ) - ' a ' ] ++ ;
PriorityQueue < Key > pq = new PriorityQueue < > ( new KeyComparator ( ) ) ; for ( char c = ' a ' ; c <= ' z ' ; c ++ ) { int val = c - ' a ' ; if ( count [ val ] > 0 ) pq . add ( new Key ( count [ val ] , c ) ) ; }
str = " " ;
Key prev = new Key ( - 1 , ' # ' ) ;
while ( pq . size ( ) != 0 ) {
Key k = pq . peek ( ) ; pq . poll ( ) ; str = str + k . ch ;
if ( prev . freq > 0 ) pq . add ( prev ) ;
( k . freq ) -- ; prev = k ; }
if ( n != str . length ( ) ) System . out . println ( " ▁ Not ▁ valid ▁ String ▁ " ) ;
else System . out . println ( str ) ; }
public static void main ( String args [ ] ) { String str = " bbbaa " ; rearrangeString ( str ) ; } }
public class GFG {
...
import java . util . HashMap ; import java . util . Map ; class GFG {
static int subset ( int arr [ ] , int n ) {
HashMap < Integer , Integer > mp = new HashMap < > ( ) ; for ( int i = 0 ; i < n ; i ++ ) mp . put ( arr [ i ] , mp . get ( arr [ i ] ) == null ? 1 : mp . get ( arr [ i ] ) + 1 ) ;
int res = 0 ; for ( Map . Entry < Integer , Integer > entry : mp . entrySet ( ) ) res = Math . max ( res , entry . getValue ( ) ) ; return res ; }
public static void main ( String [ ] args ) { int arr [ ] = { 5 , 6 , 9 , 3 , 4 , 3 , 4 } ; int n = arr . length ; System . out . println ( subset ( arr , n ) ) ; } }
...
import java . util . ArrayList ; import java . util . Collections ; import java . util . Comparator ; import java . util . Stack ; class GFG { static class Pair { int first , second ; public Pair ( int first , int second ) { this . first = first ; this . second = second ; } }
...
import java . io . * ; import java . util . Arrays ; class GFG {
...
import java . util . HashMap ; import java . lang . Math ;
...
import java . util . * ; class GFG { static int sz = 20 ; static int sqr = ( int ) ( Math . sqrt ( sz ) ) + 1 ;
...
if ( y <= Math . sqrt ( N ) ) { System . out . print ( dp [ x ] [ y ] + " ▁ " ) ; continue ; }
int sum = 0 ;
while ( x < N ) {
sum += arr [ x ] ;
...
import java . util . * ; class GFG { static final int MAX = 500 ;
...
public static void main ( String [ ] args ) { int a [ ] = { 7 , 2 , 3 , 0 , 5 , 10 , 3 , 12 , 18 } ; int n = a . length ; buildSparseTable ( a , n ) ; System . out . print ( query ( 0 , 2 ) + "NEW_LINE"); System . out . print ( query ( 1 , 3 ) + "NEW_LINE"); System . out . print ( query ( 4 , 5 ) + "NEW_LINE"); } }
class GFG {
...
import java . io . * ; import java . util . * ; class GFG { static class R { int x , y ; public R ( int x , int y ) { this . x = x ; this . y = y ; } }
...
import java . util . Vector ; public class GFG {
...
class GFG {
public static Integer longestSubSequence ( int [ ] [ ] A , int N , int ind , int lastf , int lasts ) { ind = ( ind > 0 ? ind : 0 ) ; lastf = ( lastf > 0 ? lastf : Integer . MIN_VALUE ) ; lasts = ( lasts > 0 ? lasts : Integer . MAX_VALUE ) ;
if ( ind == N ) return 0 ;
...
solve ( P , N ) ; } }
import java . util . * ; class GFG {
static void SieveOfEratosthenes ( int n , HashSet < Integer > allPrimes ) {
boolean [ ] prime = new boolean [ n + 1 ] ; Arrays . fill ( prime , true ) ; for ( int p = 2 ; p * p <= n ; p ++ ) {
if ( prime [ p ] == true ) {
for ( int i = p * p ; i <= n ; i += p ) prime [ i ] = false ; } }
for ( int p = 2 ; p <= n ; p ++ ) if ( prime [ p ] ) allPrimes . add ( p ) ; }
static int countInterestingPrimes ( int n ) {
HashSet < Integer > allPrimes = new HashSet < Integer > ( ) ; SieveOfEratosthenes ( n , allPrimes ) ;
HashSet < Integer > intersetingPrimes = new HashSet < Integer > ( ) ; Vector < Integer > squares = new Vector < Integer > ( ) , quadruples = new Vector < Integer > ( ) ;
for ( int i = 1 ; i * i <= n ; i ++ ) { squares . add ( i * i ) ; }
for ( int i = 1 ; i * i * i * i <= n ; i ++ ) { quadruples . add ( i * i * i * i ) ; }
for ( int a : squares ) { for ( int b : quadruples ) { if ( allPrimes . contains ( a + b ) ) intersetingPrimes . add ( a + b ) ; } }
...
if ( ( c . charAt ( i ) + co ) <= 122 ) s [ i ] = ( char ) ( ( int ) c . charAt ( i ) + co ) ; else s [ i ] = ( char ) ( ( int ) c . charAt ( i ) + co - 26 ) ; }
...
import java . lang . Math ; public class GfG {
...
import java . lang . * ; import java . io . * ; import java . util . * ; class GFG {
...
import java . text . ParseException ; class GFG {
...
if ( array [ mid ] < 0 ) {
if ( mid + 1 < array . length && array [ mid + 1 ] >= 0 ) { return mid ; }
...
for ( int i = 0 ; i < n ; i ++ ) {
if ( M [ i ] [ 0 ] >= 0 ) { break ; }
nextEnd = getLastNegativeIndex ( M [ i ] , 0 , nextEnd ) ; count += nextEnd + 1 ; } return count ; }
public static void main ( String [ ] args ) { int M [ ] [ ] = { { - 3 , - 2 , - 1 , 1 } , { - 2 , 2 , 3 , 4 } , { 4 , 5 , 7 , 8 } } ; int r = M . length ; int c = M [ 0 ] . length ; System . out . println ( countNegative ( M , r , c ) ) ; } }
import java . io . * ; import java . util . * ; class GFG {
...
public static void main ( String args [ ] ) { int mat [ ] [ ] = { { 10 , 20 , 30 , 40 } , { 15 , 25 , 35 , 45 } , { 27 , 29 , 37 , 48 } , { 32 , 33 , 39 , 50 } } ; printSorted ( mat ) ; } }
class GFG {
...
import java . io . * ; class MaxRepeating {
...
import java . util . Arrays ; import java . util . Collections ; public class GfG {
...
import java . util . * ; import java . lang . * ; import java . io . * ; class LinkedList {
...
import java . util . * ; import java . lang . * ; import java . io . * ; class Array_Rotation {
...
import java . util . * ; class GFG {
static boolean check ( int K , int n , int m , ArrayList < Integer > arr , int good_share ) { int candies = n , taken = 0 ; while ( candies > 0 ) {
taken += Math . min ( K , candies ) ; candies -= Math . min ( K , candies ) ;
for ( int j = 0 ; j < m ; j ++ ) {
int consume = ( arr . get ( j ) * candies ) / 100 ;
candies -= consume ; } }
...
int good_share = ( int ) Math . ceil ( ( N * 1.0 ) / ( ( M + 1 ) * 1.0 ) ) ; int lo = 1 , hi = N ;
while ( lo < hi ) {
int mid = ( lo + hi ) / 2 ;
...
System . out . print ( hi ) ; }
public static void main ( String [ ] args ) { int N = 13 , M = 1 ; ArrayList < Integer > arr = new ArrayList < Integer > ( ) ; arr . add ( 50 ) ; minimumK ( arr , N , M ) ; } }
import java . util . * ; class GFG {
...
static int cntDisPairs ( int arr [ ] , int N , int K ) {
int cntPairs = 0 ;
HashMap < Integer , Integer > cntFre = new HashMap < Integer , Integer > ( ) ; for ( int i = 0 ; i < N ; i ++ ) {
if ( cntFre . containsKey ( arr [ i ] ) ) cntFre . put ( arr [ i ] , cntFre . get ( arr [ i ] ) + 1 ) ; else cntFre . put ( arr [ i ] , 1 ) ; }
for ( Map . Entry < Integer , Integer > it : cntFre . entrySet ( ) ) {
int i = it . getKey ( ) ;
if ( 2 * i == K ) {
if ( cntFre . get ( i ) > 1 ) cntPairs += 2 ; } else { if ( cntFre . containsKey ( K - i ) ) {
cntPairs += 1 ; } } }
...
ArrayList < Integer > pref = new ArrayList < > ( ) ; pref . add ( 0 ) ;
for ( int i = 0 ; i < n ; i ++ ) pref . add ( ( a [ i ] + pref . get ( i ) ) % k ) ;
cnt . put ( 0 , 1 ) ; for ( int i = 1 ; i <= n ; i ++ ) {
int remIdx = i - k ; if ( remIdx >= 0 ) { if ( cnt . containsKey ( ( pref . get ( remIdx ) - remIdx % k + k ) % k ) ) cnt . put ( ( pref . get ( remIdx ) - remIdx % k + k ) % k , cnt . get ( ( pref . get ( remIdx ) - remIdx % k + k ) % k ) - 1 ) ; else cnt . put ( ( pref . get ( remIdx ) - remIdx % k + k ) % k , - 1 ) ; }
if ( cnt . containsKey ( ( pref . get ( i ) - i % k + k ) % k ) ) ans += cnt . get ( ( pref . get ( i ) - i % k + k ) % k ) ;
if ( cnt . containsKey ( ( pref . get ( i ) - i % k + k ) % k ) ) cnt . put ( ( pref . get ( i ) - i % k + k ) % k , cnt . get ( ( pref . get ( i ) - i % k + k ) % k ) + 1 ) ; else cnt . put ( ( pref . get ( i ) - i % k + k ) % k , 1 ) ; }
System . out . println ( ans ) ; }
public static void main ( String [ ] args ) throws java . lang . Exception {
int arr [ ] = { 2 , 3 , 5 , 3 , 1 , 5 } ;
int N = arr . length ;
...
class GFG { static int max_element ( int arr [ ] , int n ) { int max = arr [ 0 ] ; for ( int i = 1 ; i < n ; i ++ ) { if ( max < arr [ i ] ) max = arr [ i ] ; } return max ; }
...
import java . util . * ; class GFG { static int MAX = 100001 ;
...
class GFG { static final long M = 1000000007 ;
...
public static void main ( String [ ] args ) { int n = 150 ; System . out . print ( findValue ( n ) ) ; } }
import java . util . * ; class GFG {
...
class GFG { final static int INT_MIN = Integer . MIN_VALUE ;
static int maxSum ( int a [ ] , int n ) {
...
import java . util . * ; class GFG { static class pair { float first , second ; pair ( float first , float second ) { this . first = first ; this . second = second ; } }
...
import java . util . Scanner ; class contiguous_subarrays {
...
class GFG {
static int nearest ( int n ) {
int prevCube = ( int ) Math . cbrt ( n ) ; int nextCube = prevCube + 1 ; prevCube = prevCube * prevCube * prevCube ; nextCube = nextCube * nextCube * nextCube ;
int ans = ( n - prevCube ) < ( nextCube - n ) ? ( prevCube - n ) : ( nextCube - n ) ;
return ans ; }
public static void main ( String [ ] args ) { int n = 25 ; System . out . println ( nearest ( n ) ) ; n = 27 ; System . out . println ( nearest ( n ) ) ; n = 40 ; System . out . println ( nearest ( n ) ) ; } }
...
import java . lang . Math ; import java . io . * ; class GFG {
...
import java . io . * ; class Nth { public int nthTerm ( int N ) {
...
int sum = x * x * x + y * y * y ;
if ( sum != n ) continue ;
if ( s . containsKey ( sum ) ) { System . out . print ( " ( " + s . get ( sum ) . first + " , ▁ " + s . get ( sum ) . second + " ) ▁ and ▁ ( " + x + " , ▁ " + y + " ) " + "NEW_LINE"); } else
...
import java . util . Vector ; class Test {
...
TrieNode ( ) { children = new TrieNode [ 2 ] ; this . children [ 0 ] = null ; this . children [ 1 ] = null ; this . sum_of_indexes = 0 ; this . number_of_indexes = 0 ; } } ;
static void insert ( TrieNode node , int num , int index ) {
for ( int bits = lg ; bits >= 0 ; bits -- ) {
int curr_bit = ( num >> bits ) & 1 ;
if ( node . children [ curr_bit ] == null ) { node . children [ curr_bit ] = new TrieNode ( ) ; } node = node . children [ curr_bit ] ; }
node . sum_of_indexes += index ;
node . number_of_indexes ++ ; }
static int query ( TrieNode node , int num , int index ) {
for ( int bits = lg ; bits >= 0 ; bits -- ) {
int curr_bit = ( num >> bits ) & 1 ;
if ( node . children [ curr_bit ] == null ) { return 0 ; } node = node . children [ curr_bit ] ; }
...
public static void main ( String args [ ] ) {
int arr [ ] = { 5 , 2 , 7 } ; int n = arr . length ; System . out . println ( no_of_triplets ( arr , n ) ) ; } }
...
import java . util . * ; class GFG { static int coinchange ( int [ ] a , int v , int n , int [ ] [ ] dp ) { if ( v == 0 ) return dp [ n ] [ v ] = 1 ; if ( n == 0 ) return 0 ; if ( dp [ n ] [ v ] != - 1 ) return dp [ n ] [ v ] ; if ( a [ n - 1 ] <= v ) {
...
import java . util . * ; import java . lang . * ; public class BinaryS {
...
printClosest ( N , X ) ; } }
import java . io . Console ; import java . util . HashMap ; import java . util . Map ; class GFG {
...
import java . awt . Point ; import java . util . HashSet ; class GFG {
...
public static void main ( String [ ] args ) { int a = 20 , b = 30 , c = 40 ; System . out . println ( middleOfThree ( a , b , c ) ) ; } }
import java . util . * ; import java . util . Arrays ; class GFG {
...
import java . util . * ; class GFG {
public static void main ( String [ ] args ) { int n = 5 ; int [ ] arr = { 4 , 7 , 2 , 3 , 9 } ; System . out . println ( minOperations ( arr , n ) ) ; }
public static int minOperations ( int [ ] arr , int n ) {
Pair [ ] num = new Pair [ n ] ; for ( int i = 0 ; i < n ; i ++ ) { num [ i ] = new Pair ( arr [ i ] , i ) ; }
Arrays . sort ( num , ( Pair a , Pair b ) -> a . val - b . val ) ;
int res = 1 ; int streak = 1 ; int prev = num [ 0 ] . idx ; for ( int i = 1 ; i < n ; i ++ ) { if ( prev < num [ i ] . idx ) { res ++ ;
streak = Math . max ( res , streak ) ; } else res = 1 ; prev = num [ i ] . idx ; }
return n - streak ; } }
...
for ( int i = 0 ; i < dp . length ; i ++ ) for ( int j = 0 ; j < dp [ 0 ] . length ; j ++ ) for ( int k = 0 ; k < dp [ j ] [ 0 ] . length ; k ++ ) dp [ i ] [ j ] [ k ] = - 1 ;
...
static void sortBySetBitCount ( int [ ] arr , int n ) { ArrayList < ArrayList < Integer > > count = new ArrayList < ArrayList < Integer > > ( ) ;
for ( int i = 0 ; i < n ; ++ i ) { count . add ( new ArrayList < Integer > ( Arrays . asList ( ( - 1 ) * setBitCount ( arr [ i ] ) , arr [ i ] ) ) ) ; } Collections . sort ( count , new Comparator < ArrayList < Integer > > ( ) { @ Override public int compare ( ArrayList < Integer > o1 , ArrayList < Integer > o2 ) { return o1 . get ( 0 ) . compareTo ( o2 . get ( 0 ) ) ; } } ) ; for ( int i = 0 ; i < count . size ( ) ; i ++ ) { System . out . print ( count . get ( i ) . get ( 1 ) + " ▁ " ) ; } }
public static void main ( String [ ] args ) { int arr [ ] = { 1 , 2 , 3 , 4 , 5 , 6 } ; int n = arr . length ; sortBySetBitCount ( arr , n ) ; } }
public class GFG {
