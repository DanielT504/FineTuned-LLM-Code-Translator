...
encrypt ( input ) ; }
#include <stdio.h> NEW_LINE #include <string.h> NEW_LINE #include <stdbool.h>
...
#include <stdio.h> NEW_LINE #include <string.h> NEW_LINE #define MAX_LEN  100
...
#include <limits.h> NEW_LINE #include <stdio.h> NEW_LINE #include <string.h> NEW_LINE int min ( int a , int b ) { return ( a < b ) ? a : b ; }
...
printf ( " a ▁ = ▁ % d , ▁ b ▁ = ▁ % d STRNEWLINE " , a , b ) ; printf ( " a & b ▁ = ▁ % d STRNEWLINE " , a & b ) ;
printf ( " a ▁ b ▁ = ▁ % d STRNEWLINE " , a b ) ;
printf ( " a ^ b ▁ = ▁ % d STRNEWLINE " , a ^ b ) ;
printf ( " ~ a ▁ = ▁ % d STRNEWLINE " , a = ~ a ) ;
printf ( " b < < 1 ▁ = ▁ % d STRNEWLINE " , b << 1 ) ;
printf ( " b > > 1 ▁ = ▁ % d STRNEWLINE " , b >> 1 ) ; return 0 ; }
#include <stdio.h>
...
#include <stdio.h> NEW_LINE #define COINS  9 NEW_LINE #define MAX  20
...
#include <stdio.h> NEW_LINE #include <limits.h> NEW_LINE #include <string.h>
...
#include <stdio.h> NEW_LINE #define MAX  100000
...
#include <math.h> NEW_LINE #include <stdio.h> NEW_LINE #include <string.h>
...
#include <math.h> NEW_LINE #include <stdio.h> NEW_LINE #include <stdlib.h> NEW_LINE #include <string.h>
void print ( int * arr , int n ) { for ( int i = 0 ; i < n ; i ++ ) { printf ( " % d , ▁ " , arr [ i ] ) ; } printf ( " STRNEWLINE " ) ; }
int digit_at ( int x , int d ) { return ( int ) ( x / pow ( 10 , d - 1 ) ) % 10 ; }
void MSD_sort ( int * arr , int lo , int hi , int d ) {
if ( hi <= lo d < 1 ) { return ; } int count [ 10 + 2 ] = { 0 } ;
int temp [ n ] ;
...
#include <stddef.h> NEW_LINE #include <stdio.h> NEW_LINE #define __INSERT_THRESH  5 NEW_LINE #define __swap ( x , y )  (t = *(x), *(x) = *(y), *(y) = t)
...
merge ( a , m , n - m ) ; }
void print_array ( int a [ ] , size_t n ) { if ( n > 0 ) { printf ( " % d " , a [ 0 ] ) ; for ( size_t i = 1 ; i < n ; i ++ ) printf ( " ▁ % d " , a [ i ] ) ; } printf ( " STRNEWLINE " ) ; }
...
# include <stdio.h> NEW_LINE # include <stdlib.h> NEW_LINE # include <string.h> NEW_LINE # define NO_OF_CHARS  256
void print ( char * list [ ] , char * word , int list_size ) {
int * map = ( int * ) calloc ( sizeof ( int ) , NO_OF_CHARS ) ; int i , j , count , word_size ;
for ( i = 0 ; * ( word + i ) ; i ++ ) map [ * ( word + i ) ] = 1 ;
word_size = strlen ( word ) ;
for ( i = 0 ; i < list_size ; i ++ ) { for ( j = 0 , count = 0 ; * ( list [ i ] + j ) ; j ++ ) { if ( map [ * ( list [ i ] + j ) ] ) { count ++ ;
map [ * ( list [ i ] + j ) ] = 0 ; } } if ( count == word_size ) printf ( " % s " , list [ i ] ) ;
for ( j = 0 ; * ( word + j ) ; j ++ ) map [ * ( word + j ) ] = 1 ; } }
int main ( ) { char str [ ] = " sun " ; char * list [ ] = { " geeksforgeeks " , " unsorted " , " sunday " , " just " , " sss " } ; print ( list , str , 5 ) ; getchar ( ) ; return 0 ; }
...
#include <stdio.h> NEW_LINE #include <limits.h> NEW_LINE #include <string.h> NEW_LINE int min ( int a , int b ) { return a < b ? a : b ; }
int findMinInsertions ( char str [ ] , int l , int h ) {
if ( l > h ) return INT_MAX ; if ( l == h ) return 0 ; if ( l == h - 1 ) return ( str [ l ] == str [ h ] ) ? 0 : 1 ;
return ( str [ l ] == str [ h ] ) ? findMinInsertions ( str , l + 1 , h - 1 ) : ( min ( findMinInsertions ( str , l , h - 1 ) , findMinInsertions ( str , l + 1 , h ) ) + 1 ) ; }
int main ( ) { char str [ ] = " geeks " ; printf ( " % d " , findMinInsertions ( str , 0 , strlen ( str ) - 1 ) ) ; return 0 ; }
...
for ( int i = 1 ; i <= m . size ; i ++ ) { for ( int j = 1 ; j <= m . size ; j ++ ) { if ( i >= j ) printf ( " % d ▁ " , m . A [ ( ( m . size ) * ( j - 1 ) - ( ( ( j - 2 ) * ( j - 1 ) ) / 2 ) + ( i - j ) ) ] ) ; else printf ( "0 ▁ " ) ; } printf ( " STRNEWLINE " ) ; } }
...
int main ( ) { int arr [ ] = { 1 , 2 , 3 , 4 , 5 } ; int N = sizeof ( arr ) / sizeof ( arr [ 0 ] ) ; printAlter ( arr , N ) ; }
#include <stdio.h> NEW_LINE #include <math.h> NEW_LINE int sort ( int a [ ] , int n ) { int i , j , tmp ; for ( i = 0 ; i < n ; i ++ ) { for ( j = i + 1 ; j < n ; j ++ ) { if ( a [ j ] < a [ i ] ) { tmp = a [ i ] ; a [ i ] = a [ j ] ; a [ j ] = tmp ; } } } return 0 ; }
...
if ( swapped == 0 ) break ; } }
void printList ( struct Node * n ) { while ( n != NULL ) { printf ( " % d ▁ - > ▁ " , n -> data ) ; n = n -> next ; } printf ( " STRNEWLINE " ) ; }
...
printf ( " Linked ▁ list ▁ before ▁ sorting STRNEWLINE " ) ; printList ( start ) ;
bubbleSort ( & start , list_size ) ;
printf ( " Linked ▁ list ▁ after ▁ sorting STRNEWLINE " ) ; printList ( start ) ; return 0 ; }
#include <stdio.h>
...
#include <stdio.h> NEW_LINE int partition ( int * arr , int low , int high , int * lp ) ; void swap ( int * a , int * b ) { int temp = * a ; * a = * b ; * b = temp ; } void DualPivotQuickSort ( int * arr , int low , int high ) { if ( low < high ) {
...
return g ; }
int main ( ) { int arr [ ] = { 24 , 8 , 42 , 75 , 29 , 77 , 38 , 57 } ; DualPivotQuickSort ( arr , 0 , 7 ) ; printf ( " Sorted ▁ array : ▁ " ) ; for ( int i = 0 ; i < 8 ; i ++ ) printf ( " % d ▁ " , arr [ i ] ) ; printf ( " STRNEWLINE " ) ; return 0 ; }
#include <stdio.h>
...
#include <stdio.h> NEW_LINE #include <string.h> NEW_LINE #include <stdlib.h> NEW_LINE #define ARRAY_SIZE ( a )  (sizeof(a) / sizeof(*a))
int internalSearch ( char * needle , int row , int col , char * * hay , int row_max , int col_max ) { int found = 0 ; if ( row >= 0 && row <= row_max && col >= 0 && col <= col_max && * needle == hay [ row ] [ col ] ) { char match = * needle ++ ; hay [ row ] [ col ] = 0 ; if ( * needle == 0 ) { found = 1 ; } else {
found += internalSearch ( needle , row , col + 1 , hay , row_max , col_max ) ; found += internalSearch ( needle , row , col - 1 , hay , row_max , col_max ) ; found += internalSearch ( needle , row + 1 , col , hay , row_max , col_max ) ; found += internalSearch ( needle , row - 1 , col , hay , row_max , col_max ) ; } hay [ row ] [ col ] = match ; } return found ; }
int searchString ( char * needle , int row , int col , char * * str , int row_count , int col_count ) { int found = 0 ; int r , c ; for ( r = 0 ; r < row_count ; ++ r ) { for ( c = 0 ; c < col_count ; ++ c ) { found += internalSearch ( needle , r , c , str , row_count - 1 , col_count - 1 ) ; } } return found ; }
int main ( void ) { char needle [ ] = " MAGIC " ; char * input [ ] = { " BBABBM " , " CBMBBA " , " IBABBG " , " GOZBBI " , " ABBBBC " , " MCIGAM " } ; char * str [ ARRAY_SIZE ( input ) ] ; int i ; for ( i = 0 ; i < ARRAY_SIZE ( input ) ; ++ i ) { str [ i ] = malloc ( strlen ( input [ i ] ) ) ; strcpy ( str [ i ] , input [ i ] ) ; } printf ( " count : ▁ % d STRNEWLINE " , searchString ( needle , 0 , 0 , str , ARRAY_SIZE ( str ) , strlen ( str [ 0 ] ) ) ) ; return 0 ; }
#include <stdbool.h> NEW_LINE #include <stdio.h>
bool isBalanced ( char exp [ ] ) {
bool flag = true ; int count = 0 ;
for ( int i = 0 ; exp [ i ] != ' \0' ; i ++ ) { if ( exp [ i ] == ' ( ' ) { count ++ ; } else {
count -- ; } if ( count < 0 ) {
flag = false ; break ; } }
if ( count != 0 ) { flag = false ; } return flag ; }
int main ( ) { char exp1 [ ] = " ( ( ( ) ) ) ( ) ( ) " ; if ( isBalanced ( exp1 ) ) printf ( " Balanced ▁ STRNEWLINE " ) ; else printf ( " Not ▁ Balanced ▁ STRNEWLINE " ) ; char exp2 [ ] = " ( ) ) ( ( ( ) ) " ; if ( isBalanced ( exp2 ) ) printf ( " Balanced ▁ STRNEWLINE " ) ; else printf ( " Not ▁ Balanced ▁ STRNEWLINE " ) ; return 0 ; }
#include <stdio.h> NEW_LINE #define MAX_SIZE  100
void shortestToChar ( char s [ ] , char c , int * res ) {
int list [ MAX_SIZE ] ;
int len = 0 ;
int l = 0 ;
while ( s [ len ] != ' \0' ) { if ( s [ len ] == c ) { list [ l ] = len ; l ++ ; } len ++ ; } int p1 , p2 , v1 , v2 ;
l = l - 1 ;
p1 = 0 ; p2 = l > 0 ? 1 : 0 ;
for ( int i = 0 ; i < len ; i ++ ) {
...
if ( i - v1 < v2 - i ) { res [ i ] = ( i - v1 ) ; }
else { res [ i ] = ( v2 - i ) ;
p1 = p2 ; p2 = p2 < l ? ( p2 + 1 ) : p2 ; } }
else { res [ i ] = ( i - v2 ) ; } } }
int main ( ) { char s [ ] = " geeksforgeeks " ; char c = ' e ' ; int res [ MAX_SIZE ] ; shortestToChar ( s , c , res ) ; int i = 0 ; while ( s [ i ] != ' \0' ) printf ( " % d ▁ " , res [ i ++ ] ) ; return 0 ; }
#include <stdio.h> NEW_LINE #include <string.h>
...
int main ( ) {
int N = 153 ;
if ( isArmstrong ( N ) == 1 ) printf ( " True STRNEWLINE " ) ; else printf ( " False STRNEWLINE " ) ; return 0 ; }
#include <stdio.h>
void primeInRange ( int L , int R ) { int i , j , flag ;
for ( i = L ; i <= R ; i ++ ) {
if ( i == 1 i == 0 ) continue ;
flag = 1 ;
for ( j = 2 ; j <= i / 2 ; ++ j ) { if ( i % j == 0 ) { flag = 0 ; break ; } }
if ( flag == 1 ) printf ( " % d ▁ " , i ) ; } }
int main ( ) {
int L = 1 ; int R = 10 ;
primeInRange ( L , R ) ; return 0 ; }
#include <stdio.h> NEW_LINE #define lli  long long int
...
inch_sum += x % 12 ;
feet_sum += x / 12 ; }
printf ( " Feet ▁ Sum : ▁ % d STRNEWLINE " , feet_sum ) ; printf ( " Inch ▁ Sum : ▁ % .2f " , inch_sum ) ; }
int main ( ) {
struct InchFeet arr [ ] = { { 10 , 3.7 } , { 10 , 5.5 } , { 6 , 8.0 } } ; int N = sizeof ( arr ) / sizeof ( arr [ 0 ] ) ;
...
#include <omp.h> NEW_LINE #include <stdio.h> NEW_LINE #include <stdlib.h> NEW_LINE #include <time.h>
...
printf ( " Final ▁ Estimation ▁ of ▁ Pi ▁ = ▁ % f STRNEWLINE " , pi ) ; }
int main ( ) {
int N = 100000 ; int K = 8 ;
monteCarlo ( N , K ) ; }
#include <stdio.h> NEW_LINE int main ( void ) {
int x , y ;
int result ; x = -3 ; y = 4 ; result = x % y ; printf ( " % d " , result ) ; x = 4 ; y = -2 ; result = x % y ; printf ( " % d " , result ) ; x = -3 ; y = -4 ; result = x % y ; printf ( " % d " , result ) ; return 0 ; }
...
for ( row_index = 0 ; row_index < size ; row_index ++ ) { for ( column_index = 0 ; column_index < size ; column_index ++ ) { matrix [ row_index ] [ column_index ] = ++ x ; } }
printf ( " The ▁ matrix ▁ is STRNEWLINE " ) ; for ( row_index = 0 ; row_index < size ; row_index ++ ) { for ( column_index = 0 ; column_index < size ; column_index ++ ) { printf ( " % d TABSYMBOL " , matrix [ row_index ] [ column_index ] ) ; } printf ( " STRNEWLINE " ) ; }
printf ( " Corner Elements are : " for ( row_index = 0 ; row_index < size ; row_index ++ ) { for ( column_index = 0 ; column_index < size ; column_index ++ ) {
if ( ( row_index == 0 row_index == size - 1 ) && ( column_index == 0 column_index == size - 1 ) ) printf ( " % d , ▁ " , matrix [ row_index ] [ column_index ] ) ; } } return 0 ; }
#include <stdio.h> NEW_LINE #include <math.h>
...
#include <stdio.h> NEW_LINE #include <string.h> NEW_LINE #define MAX  100
...
#include <stdio.h> NEW_LINE long long int canBeSumofConsec ( long long int n ) {
n = 2 * n ;
return ( ( n & ( n - 1 ) ) != 0 ) ; } int main ( ) { long long int n = 10 ; printf ( " % lld " , canBeSumofConsec ( n ) ) ; }
#include <stdio.h> NEW_LINE #include <stdlib.h> NEW_LINE #include <stdbool.h> NEW_LINE #define COMPUTER  1 NEW_LINE #define HUMAN  2
struct move { int pile_index ; int stones_removed ; } ;
void showPiles ( int piles [ ] , int n ) { int i ; printf ( " Current ▁ Game ▁ Status ▁ - > ▁ " ) ; for ( i = 0 ; i < n ; i ++ ) printf ( " % d ▁ " , piles [ i ] ) ; printf ( " STRNEWLINE " ) ; return ; }
bool gameOver ( int piles [ ] , int n ) { int i ; for ( i = 0 ; i < n ; i ++ ) if ( piles [ i ] != 0 ) return ( false ) ; return ( true ) ; }
void declareWinner ( int whoseTurn ) { if ( whoseTurn == COMPUTER ) printf ( " HUMAN won " else printf ( " COMPUTER won " return ; }
...
knowWinnerBeforePlaying ( piles , n , COMPUTER ) ;
return ( 0 ) ; }
#include <math.h> NEW_LINE #include <stdio.h> NEW_LINE #include <stdlib.h>
...
#include <stdio.h> NEW_LINE int add ( int x , int y ) { return printf ( " % * c % * c " , x , ' ▁ ' , y , ' ▁ ' ) ; }
...
int main ( ) { int n = 19 ; if ( isMultipleof5 ( n ) == true ) printf ( " % d ▁ is ▁ multiple ▁ of ▁ 5 STRNEWLINE " , n ) ; else printf ( " % d ▁ is ▁ not ▁ a ▁ multiple ▁ of ▁ 5 STRNEWLINE " , n ) ; return 0 ; }
#include <inttypes.h> NEW_LINE #include <stdio.h> NEW_LINE #include <stdlib.h>
struct Node {
int data ;
struct Node * nxp ; } ;
struct Node * XOR ( struct Node * a , struct Node * b ) { return ( struct Node * ) ( ( uintptr_t ) ( a ) ^ ( uintptr_t ) ( b ) ) ; }
...
next = XOR ( prev , curr -> nxp ) ;
prev = curr ;
curr = next ; } printf ( " STRNEWLINE " ) ; }
struct Node * reverse ( struct Node * * head ) {
struct Node * curr = * head ; if ( curr == NULL ) return NULL ; else {
struct Node * prev = NULL ;
struct Node * next ; while ( XOR ( prev , curr -> nxp ) != NULL ) {
next = XOR ( prev , curr -> nxp ) ;
prev = curr ;
curr = next ; }
...
int main ( ) {
struct Node * head = NULL ; insert ( & head , 10 ) ; insert ( & head , 20 ) ; insert ( & head , 30 ) ; insert ( & head , 40 ) ;
printf ( " XOR ▁ linked ▁ list : ▁ " ) ; printList ( & head ) ; reverse ( & head ) ; printf ( " Reversed ▁ XOR ▁ linked ▁ list : ▁ " ) ; printList ( & head ) ; return ( 0 ) ; }
#include <inttypes.h> NEW_LINE #include <stdio.h> NEW_LINE #include <stdlib.h>
struct Node {
int data ;
struct Node * nxp ; } ;
...
next = XOR ( prev , curr -> nxp ) ;
prev = curr ;
curr = next ; count ++ ; } if ( curr == NULL && count < N ) { printf ( " Wrong ▁ Input " ) ; return ( uintptr_t ) 0 ; } else { while ( curr != NULL ) {
next = XOR ( prev , curr -> nxp ) ; next1 = XOR ( prev1 , curr1 -> nxp ) ;
prev = curr ; prev1 = curr1 ;
curr = next ; curr1 = next1 ; } printf ( " % d " , curr1 -> data ) ; } }
int main ( ) {
...
prev = curr ;
curr = next ; count ++ ; }
if ( len & 1 ) { printf ( " % d " , curr -> data ) ; }
else { printf ( " % d ▁ % d " , prev -> data , curr -> data ) ; } }
int main ( ) {
...
node -> nxp = XOR ( NULL , NULL ) ;
* head = node ; }
else { printf ( " Invalid ▁ Position STRNEWLINE " ) ; } }
else {
int Pos = 1 ;
struct Node * curr = * head ;
struct Node * prev = NULL ;
...
node -> nxp = XOR ( NULL , curr ) ;
* head = node ;
node -> data = value ; } else { printf ( " Invalid ▁ Position STRNEWLINE " ) ; } } return * head ; }
void printList ( struct Node * * head ) {
struct Node * curr = * head ;
struct Node * prev = NULL ;
struct Node * next ;
...
#include <stdio.h> NEW_LINE using namespace std ; int main ( ) {
unsigned char a = 5 , b = 9 ;
printf ( " a > > 1 ▁ = ▁ % d STRNEWLINE " , a >> 1 ) ;
printf ( " b > > 1 ▁ = ▁ % d STRNEWLINE " , b >> 1 ) ; return 0 ; }
#include <stdio.h> NEW_LINE int main ( ) { int x = 19 ; unsigned long long y = 19 ; printf ( " x ▁ < < ▁ 1 ▁ = ▁ % d STRNEWLINE " , x << 1 ) ; printf ( " x ▁ > > ▁ 1 ▁ = ▁ % d STRNEWLINE " , x >> 1 ) ;
printf ( " y ▁ < < ▁ 61 ▁ = ▁ % lld STRNEWLINE " , y << 61 ) ; return 0 ; }
...
#include <stdio.h> NEW_LINE int main ( ) { int a = 10 , b = 4 ;
if ( a > b ) printf ( " a ▁ is ▁ greater ▁ than ▁ b STRNEWLINE " ) ; else printf ( " a ▁ is ▁ less ▁ than ▁ or ▁ equal ▁ to ▁ b STRNEWLINE " ) ;
if ( a >= b ) printf ( " a ▁ is ▁ greater ▁ than ▁ or ▁ equal ▁ to ▁ b STRNEWLINE " ) ; else printf ( " a ▁ is ▁ lesser ▁ than ▁ b STRNEWLINE " ) ;
if ( a < b ) printf ( " a ▁ is ▁ less ▁ than ▁ b STRNEWLINE " ) ; else printf ( " a ▁ is ▁ greater ▁ than ▁ or ▁ equal ▁ to ▁ b STRNEWLINE " ) ;
if ( a <= b ) printf ( " a ▁ is ▁ lesser ▁ than ▁ or ▁ equal ▁ to ▁ b STRNEWLINE " ) ; else printf ( " a ▁ is ▁ greater ▁ than ▁ b STRNEWLINE " ) ;
if ( a == b ) printf ( " a ▁ is ▁ equal ▁ to ▁ b STRNEWLINE " ) ; else printf ( " a ▁ and ▁ b ▁ are ▁ not ▁ equal STRNEWLINE " ) ;
if ( a != b ) printf ( " a ▁ is ▁ not ▁ equal ▁ to ▁ b STRNEWLINE " ) ; else printf ( " a ▁ is ▁ equal ▁ b STRNEWLINE " ) ; return 0 ; }
#include <stdio.h> NEW_LINE int main ( ) { int a = 10 , b = 4 , c = 10 , d = 20 ;
if ( a > b && c == d ) printf ( " a ▁ is ▁ greater ▁ than ▁ b ▁ AND ▁ c ▁ is ▁ equal ▁ to ▁ d STRNEWLINE " ) ; else printf ( " AND ▁ condition ▁ not ▁ satisfied STRNEWLINE " ) ;
if ( a > b c == d ) printf ( " a ▁ is ▁ greater ▁ than ▁ b ▁ OR ▁ c ▁ is ▁ equal ▁ to ▁ d STRNEWLINE " ) ; else printf ( " Neither ▁ a ▁ is ▁ greater ▁ than ▁ b ▁ nor ▁ c ▁ is ▁ equal ▁ " " ▁ to ▁ d STRNEWLINE " ) ;
if ( ! a ) printf ( " a ▁ is ▁ zero STRNEWLINE " ) ; else printf ( " a ▁ is ▁ not ▁ zero " ) ; return 0 ; }
...
int main ( void ) { int arr [ ] = { 12 , 12 , 14 , 90 , 14 , 14 , 14 } ; int n = sizeof ( arr ) / sizeof ( arr [ 0 ] ) ; printf ( " The ▁ odd ▁ occurring ▁ element ▁ is ▁ % d ▁ " , findOdd ( arr , n ) ) ; return 0 ; }
#include <stdio.h> NEW_LINE int main ( ) { int x = 2 , y = 5 ; ( x & y ) ? printf ( " True ▁ " ) : printf ( " False ▁ " ) ; ( x && y ) ? printf ( " True ▁ " ) : printf ( " False ▁ " ) ; return 0 ; }
#include <math.h> NEW_LINE #include <stdio.h>
int input [ 32 ] ;
int code [ 32 ] ; int ham_calc ( int , int ) ; void solve ( int input [ ] , int ) ;
int ham_calc ( int position , int c_l ) { int count = 0 , i , j ; i = position - 1 ;
while ( i < c_l ) { for ( j = i ; j < i + position ; j ++ ) {
if ( code [ j ] == 1 ) count ++ ; }
i = i + 2 * position ; } if ( count % 2 == 0 ) return 0 ; else return 1 ; }
void solve ( int input [ ] , int n ) { int i , p_n = 0 , c_l , j , k ; i = 0 ;
while ( n > ( int ) pow ( 2 , i ) - ( i + 1 ) ) { p_n ++ ; i ++ ; } c_l = p_n + n ; j = k = 0 ;
for ( i = 0 ; i < c_l ; i ++ ) {
if ( i == ( ( int ) pow ( 2 , k ) - 1 ) ) { code [ i ] = 0 ; k ++ ; }
else { code [ i ] = input [ j ] ; j ++ ; } }
for ( i = 0 ; i < p_n ; i ++ ) {
int position = ( int ) pow ( 2 , i ) ;
int value = ham_calc ( position , c_l ) ;
code [ position - 1 ] = value ; }
printf ( " The generated Code Word is : " for ( i = 0 ; i < c_l ; i ++ ) { printf ( " % d " , code [ i ] ) ; } }
void main ( ) {
input [ 0 ] = 0 ; input [ 1 ] = 1 ; input [ 2 ] = 1 ; input [ 3 ] = 1 ; int N = 4 ;
solve ( input , N ) ; }
#include <limits.h> NEW_LINE #include <stdio.h> NEW_LINE #include <math.h> NEW_LINE #define NO_OF_CHARS  256
